...
func TestArrayLiteralExpression(t *testing.T) {
	...
	if len(result.Elements) != 3 {
		t.Fatalf("array has wrong num of elements. expect=3 got=%d",
			len(result.Elements))
	}
...
func TestFunctionObject(t *testing.T) {
	...
	if len(fn.Parameters) != 1 {
		t.Fatalf("function has wrong paramaeters. Parameters=%+v",
			fn.Parameters)
	}
...
if (10 > 1) {
	...
	for _, tt := range tests {
		...
		if errObj.Message != tt.expectedString {
			t.Errorf("wrong error message. expected=%q, got=%q",
				tt.expectedString, errObj.Message)
		}
	}
}
